...
from utils import crop_img_by_zeros, norm_data
...
class CorticalProjectionPreprocessor:
    ...
    def project_data(self):
        ...
        self.archive_name = Path(self.config["gen_dir"]) / f'topomaps_cortex-views_subj-{num_subjects}_ico{self.config["spacing_ico"]}_lbl-{self.n_labels}.h5'
        ...
        for subject in self.config["subj_dict"].keys():
            ...
            self.save_dirs[subject] = Path(self.config["gen_dir"]) / fwd_fname.replace(".fif","") / f'Parcellation_{self.n_labels}-lbl'
        ...
        with h5py.File(self.archive_name, 'a') as out_file:
            
            keys = list(out_file.keys())

            for j, subject in enumerate(self.config["subj_dict"].keys()):

                fwd = self.fwds[subject]
                src = fwd['src']
                vertices = [src[0]['vertno'], src[1]['vertno']]
                save_dir = self.save_dirs[subject]

                sensors_dir = save_dir / "sensors"
                dipoles_dir = save_dir / "dipoles"

                print(f"\nGenerating topomaps and cortical views for {subject} subject:")

                for sensors_name, source_name in tqdm(list(zip((sensors_dir.iterdir()), dipoles_dir.iterdir()))):
                    
                    i = int(sensors_name.stem.split("_")[0].split("-")[-1])
                    n_lbls = source_name.stem.split("_")[-1].split("-")[-1]
                    snr = sensors_name.stem.split("_")[-1].split("-")[-1]

                    cortex_views_name = f'cortex_views_dip-{j*10000+i}_lbls-{n_lbls}_subj-{subject}'
                    topomaps_name = f'topomaps_eeg-{j*10000+i}_snr-{snr}_subj-{subject}'

                    if topomaps_name not in keys:
                        topomaps = self.project_and_interpolate_sensors(sensors_name)
                        out_file.create_dataset(topomaps_name, data=topomaps,
                                            shape=topomaps.shape, dtype=np.float32, compression="gzip", compression_opts=4)
                    
                    if cortex_views_name not in keys:
                        cortex_views = self.project_sources_onto_cortex(subject, source_name, vertices)
                        out_file.create_dataset(cortex_views_name, data=cortex_views,
                                        shape=cortex_views.shape, dtype=np.float32, compression="gzip", compression_opts=4)
                        
        print("Done!")


    def project_and_interpolate_sensors(self, sensors_path, sep_hyperplanes_coords=[0, -20, 0], normalize=True):

        sensors = np.load(sensors_path)
        if normalize:
            sensors = norm_data(sensors)
        ...
        topomaps = np.array(topomaps, dtype=np.float32)

        return topomaps


    def project_sources_onto_cortex(self, subject, sources_path, vertices):
        
        subject_name = self.config["subj_dict"][subject]["fs_name"]
        sources = np.load(sources_path)
        stc = mne.SourceEstimate(sources, vertices, subject=subject_name, tmin=0, tstep=1/1000)

        if self.onto_morph:
            morph_path = Path(self.config["morph_dir"]) / f'{subject_name}-fsaverage-ico{self.config["spacing_ico"]}.h5'
        ...
        for hemi_view_combination, loss_mask in zip(self.hemi_view_combinations, self.loss_masks):
            ...
            view = cv2.imread(temp_path, cv2.IMREAD_GRAYSCALE)
            view = crop_img_by_zeros(view, by_img=loss_mask)
            view = cv2.resize(view, (self.grid,self.grid))

            views.append(view)

        views = np.array(views, dtype=np.float32)/255

        return views


def get_cortical_loss_masks(config, hemi_view_combinations, subject='fsaverage', save_dir='', fwd=None, save=True, grid=256):
    """
